<?php
	...
	foreach ( $classes as $class ) :
		$number++
		?>
		<div class="class" data-nrc="<?php echo $class[ 'nrc' ]; ?>" style="<?php if ($number%3==0) echo 'margin-right: 0px;'; ?>">
			<div>
		...
		<?php if ( trim( $class[ 'timetable' ][0][ 'day' ] ) != '' && $class[ 'timetable' ][0][ 'day' ] != ' ' ): ?>
			...
			*/
				...
				if ( !empty( $class[ 'notes' ] ) ):
					?>
					<div class="notes" id="class-<?php echo $class[ 'nrc' ]; ?>-notes">
						<?php
							if ( md5( $class[ 'notes' ] ) == '6496af59f3e58084b2a48b4fb93bf696' ):
								echo 'Section accessible aux étudiants provenant des autres programmes.';
							else:
								echo substr( str_replace( "", "'", $class[ 'notes' ]), 0, 60 ); ?>...
								<div style="text-align: left; padding-top: 2px;">
									<a class="show-notes-link" href="javascript:displayNotes(<?php echo $class[ 'nrc' ]; ?>);">&raquo; Voir plus</a>
								</div>
								<?php
							endif;
						?>
					...
					<!--
					<div class="notes full" id="class-<?php echo $class['nrc']; ?>-notes-full" style="display: none;">
						<?php echo str_replace("", "'", $class['notes']); ?>
					</div>
					-->
					<?php
				endif;
			...
			?>
			<div class="nrc">NRC : <strong><?php echo $class[ 'nrc' ]; ?></strong>
			<div class="registration-state">
				<?php
					if ( in_array( $class[ 'nrc' ], $registeredCourses ) ) {
						...
						echo $this->Html->link( '<i class="icon-plus"></i>&nbsp;Ajouter', '#', array( 'class' => 'btn js-select-btn', 'escape' => false ) );
					}
				?>
			</div>
		</div>
